

// A large list of biology terms. Case-insensitive matching will be applied.
// This list is illustrative and can be expanded significantly.

export const BIOLOGY_TERMS: string[] = [
    // Core Biology Concepts
    "Abiotic factor",
    "Absorption",
    "Acid",
    "Acquired trait",
    "Active transport",
    "Adaptation",
    "Adaptive radiation",
    "Adenine",
    "Adenosine triphosphate (ATP)",
    "Adhesion",
    "Aerobic respiration",
    "Asexual reproduction",
    "Allele",
    "Amino acid",
    "Anaerobic respiration",
    "Anatomy",
    "Animalia",
    "Antibiotic",
    "Antibody",
    "Antigen",
    "Apoptosis",
    "Atom",
    "ATP",
    "Autotroph",
    "Auxin",
    "Base",
    "Behavior",
    "Binary fission",
    "Binomial nomenclature",
    "Biochemistry",
    "Biodiversity",
    "Biogenesis",
    "Biogeochemical cycle",
    "Biology",
    "Biomass",
    "Biome",
    "Biosphere",
    "Biotechnology",
    "Biotic factor",
    "Botany",
    "Buffer",
    "Calorie",
    "Calvin cycle",
    "Carbohydrate",
    "Carbon cycle",
    "Carbon fixation",
    "Carnivore",
    "Catalyst",
    "Cell",
    "Cell cycle",
    "Cell differentiation",
    "Cell division",
    "Cell membrane",
    "Cell respiration",
    "Cell theory",
    "Cell wall",
    "Cellular respiration",
    "Cellulose",
    "Chemical bond",
    "Chemical energy",
    "Chemical reaction",
    "Chemosynthesis",
    "Chitin",
    "Chlorophyll",
    "Chloroplast",
    "Cholesterol",
    "Chordata",
    "Chromatid",
    "Chromatin",
    "Chromosome",
    "Classification",
    "Climax community",
    "Cloning",
    "Codominance",
    "Codon",
    "Coevolution",
    "Cohesion",
    "Commensalism",
    "Community",
    "Competition",
    "Compound",
    "Concentration gradient",
    "Condensation",
    "Conservation",
    "Consumer",
    "Convergent evolution",
    "Covalent bond",
    "Crossing over",
    "Cytokinesis",
    "Cytoplasm",
    "Cytosine",
    "Cytoskeleton",
    "Cytosol",
    "Decomposer",
    "Dehydration synthesis",
    "Deletion",
    "Denaturation",
    "Density-dependent factor",
    "Density-independent factor",
    "Deoxyribonucleic acid (DNA)",
    "Deoxyribose",
    "Dependent variable",
    "Diffusion",
    "Digestion",
    "Diploid",
    "Directional selection",
    "Disruptive selection",
    "Divergent evolution",
    "DNA",
    "DNA replication",
    "Dominant allele",
    "Double helix",
    "Ecology",
    "Ecosystem",
    "Electron",
    "Element",
    "Embryo",
    "Endangered species",
    "Endemic species",
    "Endosymbiosis",
    "Energy",
    "Entropy",
    "Environment",
    "Enzyme",
    "Eukaryote",
    "Evolution",
    "Excretion",
    "Exergonic reaction",
    "Experiment",
    "Exponential growth",
    "Extinction",
    "Facilitated diffusion",
    "Fat",
    "Fatty acid",
    "Feedback inhibition",
    "Fermentation",
    "Fertilization",
    "Fitness",
    "Food chain",
    "Food web",
    "Fossil",
    "Gene",
    "Gene expression",
    "Gene flow",
    "Gene pool",
    "Gene regulation",
    "Genetic code",
    "Genetic drift",
    "Genetic engineering",
    "Genetic equilibrium",
    "Genetics",
    "Genome",
    "Genotype",
    "Genus",
    "Global warming",
    "Glucose",
    "Glycolysis",
    "Golgi apparatus",
    "Greenhouse effect",
    "Growth",
    "Guanine",
    "Habitat",
    "Half-life",
    "Haploid",
    "Hardy-Weinberg principle",
    "Herbivore",
    "Heredity",
    "Heterotroph",
    "Heterozygous",
    "Homeostasis",
    "Homologous structure",
    "Homozygous",
    "Hormone",
    "Host",
    "Hydrogen bond",
    "Hydrolysis",
    "Hydrophilic",
    "Hydrophobic",
    "Hypothesis",
    "Immunity",
    "Inbreeding",
    "Incomplete dominance",
    "Independent assortment",
    "Independent variable",
    "Inheritance",
    "Interphase",
    "Intron",
    "Ion",
    "Ionic bond",
    "Isotope",
    "Karyotype",
    "Keystone species",
    "Kingdom",
    "Krebs cycle",
    "Law of independent assortment",
    "Law of segregation",
    "Life cycle",
    "Limiting factor",
    "Lipid",
    "Logistic growth",
    "Lysosome",
    "Macroevolution",
    "Macromolecule",
    "Mass extinction",
    "Meiosis",
    "Metabolism",
    "Metamorphosis",
    "Metaphase",
    "Microevolution",
    "Microscope",
    "Mimicry",
    "Mineral",
    "Mitochondria",
    "Mitochondrion",
    "Mitosis",
    "Molecule",
    "Monohybrid cross",
    "Monomer",
    "Monosaccharide",
    "Mutation",
    "Mutualism",
    "Mycorrhiza",
    "Natural selection",
    "Niche",
    "Nitrogen cycle",
    "Nitrogen fixation",
    "Nitrogenous base",
    "Nondisjunction",
    "Nucleic acid",
    "Nucleoid",
    "Nucleolus",
    "Nucleotide",
    "Nucleus",
    "Nutrient",
    "Nutrient cycle",
    "Nutrition",
    "Omnivore",
    "Organ",
    "Organ system",
    "Organelle",
    "Organic compound",
    "Organism",
    "Osmosis",
    "Oxidation",
    "Parasitism",
    "Passive transport",
    "Pathogen",
    "PCR (Polymerase chain reaction)",
    "Pedigree",
    "pH",
    "Phenotype",
    "Phospholipid",
    "Phospholipid bilayer",
    "Photosynthesis",
    "Phylogeny",
    "Phylum",
    "Physiology",
    "Pigment",
    "Plasma membrane",
    "Plasmid",
    "Plastid",
    "Point mutation",
    "Polar molecule",
    "Polymer",
    "Polysaccharide",
    "Population",
    "Population density",
    "Population genetics",
    "Predation",
    "Primary consumer",
    "Primary productivity",
    "Prokaryote",
    "Prophase",
    "Protein",
    "Protein synthesis",
    "Protista",
    "Proton",
    "Punctuated equilibrium",
    "Punnett square",
    "Purine",
    "Pyrimidine",
    "Recessive allele",
    "Recombinant DNA",
    "Redox reaction",
    "Reduction",
    "Replication",
    "Reproduction",
    "Resource partitioning",
    "Respiration",
    "Restriction enzyme",
    "Retrovirus",
    "Ribonucleic acid (RNA)",
    "Ribose",
    "Ribosome",
    "RNA",
    "RNA polymerase",
    "Scientific method",
    "Secondary consumer",
    "Seed",
    "Segregation",
    "Selective permeability",
    "Sex chromosome",
    "Sex linkage",
    "Sexual reproduction",
    "Sexual selection",
    "Species",
    "Speciation",
    "Spontaneous generation",
    "Spore",
    "Stabilizing selection",
    "Starch",
    "Stem cell",
    "Steroid",
    "Stimulus",
    "Stomata",
    "Substrate",
    "Succession",
    "Symbiosis",
    "Taxon",
    "Taxonomy",
    "Telophase",
    "Temperature",
    "Territoriality",
    "Tertiary consumer",
    "Test cross",
    "Theory",
    "Thylakoid",
    "Thymine",
    "Tissue",
    "Trait",
    "Transcription",
    "Transfer RNA (tRNA)",
    "Transformation",
    "Transgenic organism",
    "Translation",
    "Transpiration",
    "Trophic level",
    "Uracil",
    "Vacuole",
    "Variation",
    "Vascular plant",
    "Vascular tissue",
    "Vector",
    "Vertebrate",
    "Vestigial structure",
    "Virus",
    "Vitamin",
    "Water cycle",
    "X chromosome",
    "Xylem",
    "Y chromosome",
    "Zoology",
    "Zygote",

    // Human Anatomy & Physiology
    "Abdomen",
    "Abdominal cavity",
    "Abductor muscle",
    "Accessory organ",
    "Acetylcholine",
    "Achilles tendon",
    "Acid reflux",
    "Acne",
    "Acrosome",
    "Actin",
    "Adductor muscle",
    "Adenoid",
    "Adipose tissue", // Fat tissue
    "Adrenal gland",
    "Adrenaline", // Epinephrine
    "Aerobic exercise",
    "Afferent neuron",
    "Afterbirth", // Placenta expelled
    "Agglutination",
    "Air sac", // Alveolus
    "Airway",
    "Albumen",
    "Albumin",
    "Aldosterone",
    "Alimentary canal", // Digestive tract
    "Allergy",
    "Alopecia", // Hair loss
    "Alveolus", // Plural Alveoli
    "Amniotic fluid",
    "Amniotic sac",
    "Amylase",
    "Anaerobic exercise",
    "Anal canal",
    "Analgesic",
    "Anaphase",
    "Anemia",
    "Aneurysm",
    "Angina",
    "Ankle",
    "Antacid",
    "Antagonist muscle",
    "Anterior",
    "Antibody",
    "Antidiuretic hormone (ADH)",
    "Antigen",
    "Antihistamine",
    "Antioxidant",
    "Antrum",
    "Anus",
    "Aorta",
    "Aortic valve",
    "Aphasia",
    "Apnea",
    "Apocrine gland",
    "Appendix", // Vermiform appendix
    "Aqueous humour",
    "Arachnoid mater",
    "Arch (foot)",
    "Areola",
    "Arm",
    "Armpit", // Axilla
    "Arrhythmia",
    "Arteriole",
    "Artery",
    "Arthritis",
    "Articular cartilage",
    "Articulation", // Joint
    "Ascending colon",
    "Asthma",
    "Astigmatism",
    "Atrium", // Heart chamber
    "Auditory canal", // Ear canal
    "Auditory nerve",
    "Autonomic nervous system",
    "Axilla",
    "Axon",
    "B cell",
    "Baby teeth", // Deciduous teeth
    "Back",
    "Backbone", // Vertebral column
    "Ball-and-socket joint",
    "Basal ganglia",
    "Basophil",
    "Belly", // Abdomen
    "Belly button", // Navel
    "Biceps",
    "Bicuspid valve", // Mitral valve
    "Bile",
    "Bile duct",
    "Birth canal", // Vagina
    "Bladder", // Urinary bladder
    "Blind spot",
    "Blood",
    "Blood clot",
    "Blood group",
    "Blood plasma",
    "Blood pressure",
    "Blood sugar", // Glucose level
    "Blood type",
    "Blood vessel",
    "Body",
    "Body fat",
    "Body temperature",
    "Bone",
    "Bone marrow",
    "Brain",
    "Brainstem",
    "Breast",
    "Breastbone", // Sternum
    "Breath",
    "Breathing",
    "Bronchiole",
    "Bronchus", // Plural Bronchi
    "Bruise",
    "Buccal cavity", // Mouth
    "Bulbourethral gland", // Cowper's gland
    "Bursa",
    "Buttocks",
    "Calcaneus", // Heel bone
    "Calcitonin",
    "Calf (leg)",
    "Callus",
    "Canine tooth",
    "Capillary",
    "Carbohydrate digestion",
    "Cardiac muscle",
    "Cardiac output",
    "Cardiovascular system",
    "Carpals", // Wrist bones
    "Cartilage",
    "Cecum",
    "Cell membrane",
    "Central nervous system",
    "Cerebellum",
    "Cerebral cortex",
    "Cerebrospinal fluid (CSF)",
    "Cerebrum",
    "Cerumen", // Earwax
    "Cervical vertebrae",
    "Cervix",
    "Cheek",
    "Cheekbone", // Zygomatic bone
    "Chest",
    "Chime",
    "Chin",
    "Cholesterol",
    "Choroid",
    "Chromosome",
    "Chyme",
    "Cilia",
    "Circulatory system",
    "Circumcision",
    "Clavicle", // Collarbone
    "Clitoris",
    "Coagulation", // Clotting
    "Coccyx", // Tailbone
    "Cochlea",
    "Coenzyme",
    "Collagen",
    "Collarbone", // Clavicle
    "Colon", // Large intestine part
    "Common bile duct",
    "Conception",
    "Condyle",
    "Cone (eye)",
    "Conjunctiva",
    "Connective tissue",
    "Constipation",
    "Contraction (muscle)",
    "Cornea",
    "Coronary artery",
    "Corpus callosum",
    "Corpus luteum",
    "Cortex (brain/kidney)",
    "Cortisol",
    "Costal cartilage", // Rib cartilage
    "Cowper's gland", // Bulbourethral gland
    "Cramp",
    "Cranial nerves",
    "Cranium", // Skull
    "Crown (tooth)",
    "Cuticle (nail/hair)",
    "Cyst",
    "Dandruff",
    "Deciduous teeth", // Baby teeth
    "Defecation",
    "Dehydration",
    "Deltoid muscle",
    "Dendrite",
    "Dentin",
    "Dermis",
    "Descending colon",
    "Diabetes mellitus",
    "Dialysis",
    "Diaphragm",
    "Diarrhea",
    "Diastole",
    "Diencephalon",
    "Digestive system",
    "Digestive tract",
    "Digit", // Finger or Toe
    "Dilation",
    "Distal",
    "DNA",
    "Dopamine",
    "Dorsal", // Posterior
    "Drug",
    "Duct",
    "Ductus deferens", // Vas deferens
    "Duodenum",
    "Dura mater",
    "Ear",
    "Earlobe",
    "Ear canal",
    "Eardrum", // Tympanic membrane
    "Earwax", // Cerumen
    "Eccrine gland",
    "Ectoderm",
    "Edema",
    "Efferent neuron",
    "Egg cell", // Ovum
    "Ejaculation",
    "Ejaculatory duct",
    "Elastin",
    "Elbow",
    "Electrocardiogram (ECG/EKG)",
    "Electroencephalogram (EEG)",
    "Electrolyte",
    "Embryo",
    "Embryonic development",
    "Enamel (tooth)",
    "Endocardium",
    "Endocrine gland",
    "Endocrine system",
    "Endoderm",
    "Endometrium",
    "Endorphin",
    "Endothelium",
    "Enzyme",
    "Eosinophil",
    "Epicardium",
    "Epidermis",
    "Epididymis",
    "Epiglottis",
    "Epilepsy",
    "Epinephrine", // Adrenaline
    "Epiphyseal plate", // Growth plate
    "Epithelial tissue",
    "Epithelium",
    "Erection",
    "Erythrocyte", // Red blood cell
    "Esophagus",
    "Estrogen",
    "Eustachian tube",
    "Excretory system",
    "Exhalation",
    "Exocrine gland",
    "Extensor muscle",
    "External auditory meatus", // Ear canal
    "Eye",
    "Eyeball",
    "Eyebrow",
    "Eyelash",
    "Eyelid",
    "Face",
    "Facial nerve",
    "Fallopian tube",
    "Fascia",
    "Fat", // Adipose tissue
    "Fatigue",
    "Fatty acid",
    "Feces",
    "Femoral artery",
    "Femur", // Thigh bone
    "Fertility",
    "Fertilization",
    "Fetus",
    "Fever",
    "Fiber (dietary)",
    "Fibrin",
    "Fibrinogen",
    "Fibroid",
    "Fibula", // Lower leg bone
    "Fight-or-flight response",
    "Filtrate",
    "Finger",
    "Fingernail",
    "Fissure (brain)",
    "Flaccid",
    "Flatulence",
    "Flexion",
    "Flexor muscle",
    "Fluid",
    "Follicle", // Hair follicle, Ovarian follicle
    "Follicle-stimulating hormone (FSH)",
    "Fontanelle",
    "Foot",
    "Foramen",
    "Forearm",
    "Forehead",
    "Foreskin",
    "Fossa",
    "Fracture",
    "Frenulum",
    "Frontal lobe",
    "Gallbladder",
    "Gamete",
    "Ganglion",
    "Gas exchange",
    "Gastric acid",
    "Gastric juice",
    "Gastrocnemius", // Calf muscle
    "Gastrointestinal tract",
    "Gene",
    "Genetics",
    "Genitals",
    "Germ layer",
    "Gestation",
    "Gingiva", // Gums
    "Gland",
    "Glans penis",
    "Glaucoma",
    "Glial cell",
    "Globulin",
    "Glomerulus",
    "Glottis",
    "Glucagon",
    "Glucose",
    "Gluteus maximus",
    "Glycogen",
    "Goiter",
    "Gonad", // Ovary or Testis
    "Gout",
    "Gray matter",
    "Groin",
    "Growth hormone",
    "Growth plate", // Epiphyseal plate
    "Gullet", // Esophagus
    "Gums", // Gingiva
    "Gut", // Intestine
    "Gyrus",
    "Hair",
    "Hair follicle",
    "Hamstring",
    "Hand",
    "Head",
    "Hearing",
    "Heart",
    "Heart rate",
    "Heart valve",
    "Heartbeat",
    "Heartburn",
    "Heel",
    "Heel bone", // Calcaneus
    "Hematocrit",
    "Hemoglobin",
    "Hemorrhage",
    "Hemorrhoid",
    "Hepatic artery",
    "Hepatic vein",
    "Hepatitis",
    "Hernia",
    "Hiccup",
    "Hindbrain",
    "Hinge joint",
    "Hip",
    "Hip bone", // Pelvic bone
    "Hippocampus",
    "Histamine",
    "Homeostasis",
    "Hormone",
    "Humerus", // Upper arm bone
    "Humor (eye)", // Aqueous/Vitreous
    "Hydration",
    "Hymen",
    "Hyoid bone",
    "Hyperglycemia",
    "Hypertension", // High blood pressure
    "Hyperthermia",
    "Hyperthyroidism",
    "Hypodermis",
    "Hypoglycemia",
    "Hypotension", // Low blood pressure
    "Hypothalamus",
    "Hypothermia",
    "Hypothyroidism",
    "Ileum",
    "Ilium",
    "Immune system",
    "Immunity",
    "Immunoglobulin",
    "Implantation",
    "Incisor tooth",
    "Incontinence",
    "Incus", // Anvil bone (ear)
    "Indigestion",
    "Infection",
    "Inflammation",
    "Influenza", // Flu
    "Inguinal canal",
    "Inhalation",
    "Inner ear",
    "Insomnia",
    "Insulin",
    "Integumentary system", // Skin system
    "Intercostal muscle",
    "Interferon",
    "Interneuron",
    "Intervertebral disc",
    "Intestinal juice",
    "Intestine", // Small intestine, Large intestine
    "Intracellular fluid",
    "Intravenous (IV)",
    "Involuntary muscle", // Smooth/Cardiac
    "Iris",
    "Iron",
    "Ischium",
    "Islets of Langerhans",
    "Jaundice",
    "Jaw",
    "Jawbone", // Mandible, Maxilla
    "Jejunum",
    "Joint",
    "Joint capsule",
    "Jugular vein",
    "Keratin",
    "Kidney",
    "Kidney stone",
    "Kinase",
    "Knee",
    "Kneecap", // Patella
    "Knuckle",
    "Labia majora",
    "Labia minora",
    "Labor (childbirth)",
    "Lacrimal gland", // Tear gland
    "Lactase",
    "Lactation",
    "Lactic acid",
    "Lacuna",
    "Large intestine",
    "Larynx", // Voice box
    "Lateral",
    "Latissimus dorsi",
    "Leg",
    "Lens (eye)",
    "Leukemia",
    "Leukocyte", // White blood cell
    "Libido",
    "Ligament",
    "Limb",
    "Limbic system",
    "Lip",
    "Lipase",
    "Lipid",
    "Lipoprotein",
    "Liver",
    "Lobe (brain/lung)",
    "Loins",
    "Lordosis",
    "Lower back",
    "Lower leg",
    "Lumbar vertebrae",
    "Lumen",
    "Lung",
    "Lunula",
    "Luteinizing hormone (LH)",
    "Lymph",
    "Lymph node",
    "Lymphatic system",
    "Lymphocyte",
    "Macrophage",
    "Macula",
    "Malaria",
    "Malleus", // Hammer bone (ear)
    "Mammary gland",
    "Mandible", // Lower jaw
    "Marrow", // Bone marrow
    "Mast cell",
    "Mastication", // Chewing
    "Maxilla", // Upper jaw
    "Medial",
    "Mediastinum",
    "Medulla oblongata",
    "Megakaryocyte",
    "Meiosis",
    "Melanin",
    "Melanocyte",
    "Melatonin",
    "Membrane",
    "Memory",
    "Meninges",
    "Meniscus",
    "Menopause",
    "Menstrual cycle",
    "Menstruation", // Period
    "Mesentery",
    "Mesoderm",
    "Metabolism",
    "Metacarpals", // Hand bones
    "Metaphase",
    "Metatarsals", // Foot bones
    "Microbe",
    "Microvillus",
    "Midbrain",
    "Middle ear",
    "Migraine",
    "Milk",
    "Milk teeth", // Deciduous teeth
    "Mineralocorticoid",
    "Mitochondrion",
    "Mitosis",
    "Mitral valve", // Bicuspid valve
    "Molar tooth",
    "Molecule",
    "Monocyte",
    "Mons pubis",
    "Motor neuron",
    "Mouth",
    "Mucous membrane",
    "Mucus",
    "Muscle",
    "Muscle fiber",
    "Muscle tissue",
    "Muscular system",
    "Mutation",
    "Myelin sheath",
    "Myocardium",
    "Myofibril",
    "Myoglobin",
    "Myosin",
    "Nail", // Fingernail, Toenail
    "Nasal cavity",
    "Nasal septum",
    "Nausea",
    "Navel", // Umbilicus
    "Neck",
    "Neonate", // Newborn
    "Nephron",
    "Nerve",
    "Nerve cell", // Neuron
    "Nerve impulse",
    "Nervous system",
    "Nervous tissue",
    "Neuralgia",
    "Neuroglia",
    "Neuron",
    "Neurotransmitter",
    "Neutrophil",
    "Newborn",
    "Nipple",
    "Nitrogenous waste",
    "Node of Ranvier",
    "Noradrenaline", // Norepinephrine
    "Norepinephrine",
    "Nose",
    "Nostril",
    "Notochord",
    "Nucleus (cell)",
    "Nutrient",
    "Obesity",
    "Occipital lobe",
    "Odor",
    "Olfactory bulb",
    "Olfactory nerve",
    "Oocyte",
    "Optic nerve",
    "Oral cavity", // Mouth
    "Orbit (eye socket)",
    "Organ",
    "Organ of Corti",
    "Organ system",
    "Organelle",
    "Orgasm",
    "Ossicle", // Ear bones
    "Ossification",
    "Osteoarthritis",
    "Osteoblast",
    "Osteoclast",
    "Osteocyte",
    "Osteon",
    "Osteoporosis",
    "Outer ear",
    "Ovarian follicle",
    "Ovary",
    "Ovulation",
    "Ovum", // Egg cell
    "Oxytocin",
    "Pacemaker (heart)",
    "Pain",
    "Palate",
    "Palm (hand)",
    "Pancreas",
    "Pancreatic juice",
    "Papilla",
    "Parasympathetic nervous system",
    "Parathyroid gland",
    "Parathyroid hormone (PTH)",
    "Parietal lobe",
    "Parotid gland",
    "Patella", // Kneecap
    "Pathogen",
    "Pectoralis major",
    "Pelvic cavity",
    "Pelvic girdle",
    "Pelvis",
    "Penis",
    "Pepsin",
    "Pepsinogen",
    "Peptide",
    "Perception",
    "Pericardium",
    "Periosteum",
    "Peripheral nervous system",
    "Peristalsis",
    "Peritoneum",
    "Permanent teeth",
    "Perspiration", // Sweat
    "pH",
    "Phagocyte",
    "Phalanges", // Finger/Toe bones
    "Phalanx",
    "Pharynx",
    "Phenotype",
    "Pheromone",
    "Phlegm",
    "Phobia",
    "Photoreceptor", // Rods and Cones
    "Pia mater",
    "Pineal gland",
    "Pinna", // Auricle (ear)
    "Pituitary gland",
    "Pivot joint",
    "Placenta",
    "Plague",
    "Plantar fascia",
    "Plasma",
    "Platelet",
    "Pleura",
    "Plexus",
    "Pneumonia",
    "Pons",
    "Posterior",
    "Posture",
    "Pregnancy",
    "Premolar tooth",
    "Prepuce", // Foreskin
    "Presbyopia",
    "Progesterone",
    "Prokaryote", // Added for completeness
    "Prolactin",
    "Prophase",
    "Prostate gland",
    "Prostaglandin",
    "Protein digestion",
    "Prothrombin",
    "Proximal",
    "Puberty",
    "Pubic bone", // Pubis
    "Pubis",
    "Pulmonary artery",
    "Pulmonary valve",
    "Pulmonary vein",
    "Pulp (tooth)",
    "Pulse",
    "Pupil",
    "Pyloric sphincter",
    "Pylorus",
    "Quadriceps femoris",
    "Rabies",
    "Radial artery",
    "Radius (bone)",
    "Rash",
    "Receptor",
    "Rectum",
    "Red blood cell", // Erythrocyte
    "Reflex",
    "Reflex arc",
    "Refraction",
    "Relaxation (muscle)",
    "REM sleep",
    "Renal artery",
    "Renal vein",
    "Renin",
    "Reproductive system",
    "Residual volume",
    "Respiration",
    "Respiratory system",
    "Retina",
    "Rh factor",
    "Rheumatoid arthritis",
    "Rib",
    "Rib cage",
    "Ribonucleic acid (RNA)", // Added for completeness
    "Ribosome", // Added for completeness
    "Rickets",
    "Right atrium",
    "Right ventricle",
    "Rod (eye)",
    "Root (tooth/hair)",
    "Rotator cuff",
    "Sacrum",
    "Saddle joint",
    "Sagittal plane",
    "Saliva",
    "Salivary gland",
    "Sarcolemma",
    "Sarcomere",
    "Sarcoplasmic reticulum",
    "Scalp",
    "Scapula", // Shoulder blade
    "Scar",
    "Sciatic nerve",
    "Sclera",
    "Scoliosis",
    "Scrotum",
    "Scurvy",
    "Sebaceous gland",
    "Sebum",
    "Secretion",
    "Semen",
    "Semicircular canals",
    "Seminal vesicle",
    "Seminiferous tubules",
    "Sense",
    "Sense organ",
    "Sensory neuron",
    "Septum",
    "Serotonin",
    "Serum",
    "Sesamoid bone",
    "Sex hormone",
    "Shin",
    "Shinbone", // Tibia
    "Shivering",
    "Shock",
    "Shoulder",
    "Shoulder blade", // Scapula
    "Sigmoid colon",
    "Sinoatrial (SA) node",
    "Sinus",
    "Skeletal muscle",
    "Skeletal system",
    "Skeleton",
    "Skin",
    "Skull",
    "Sleep",
    "Small intestine",
    "Smell",
    "Smooth muscle",
    "Sneezing",
    "Snoring",
    "Sodium-potassium pump",
    "Soft palate",
    "Sole (foot)",
    "Somatic cell",
    "Somatic nervous system",
    "Sound",
    "Spasm",
    "Speech",
    "Sperm",
    "Spermatogenesis",
    "Sphincter",
    "Spinal column", // Vertebral column
    "Spinal cord",
    "Spinal nerve",
    "Spine", // Vertebral column
    "Spleen",
    "Sprain",
    "Sputum",
    "Squamous cell",
    "Stapes", // Stirrup bone (ear)
    "Stem cell",
    "Sternum", // Breastbone
    "Steroid hormone",
    "Stethoscope",
    "Stimulus",
    "Stirrup bone", // Stapes (ear)
    "Stomach",
    "Stool", // Feces
    "Strain (muscle)",
    "Stress",
    "Stretch reflex",
    "Striated muscle", // Skeletal/Cardiac
    "Stroke",
    "Subcutaneous tissue",
    "Sublingual gland",
    "Submandibular gland",
    "Sudoriferous gland", // Sweat gland
    "Sulcus",
    "Superior",
    "Supination",
    "Suprarenal gland", // Adrenal gland
    "Suture (skull)",
    "Swallowing", // Deglutition
    "Sweat", // Perspiration
    "Sweat gland",
    "Sympathetic nervous system",
    "Synapse",
    "Synovial fluid",
    "Synovial joint",
    "Synovial membrane",
    "Synergist muscle",
    "Systemic circulation",
    "Systole",
    "T cell",
    "Tachycardia",
    "Tailbone", // Coccyx
    "Tarsals", // Ankle bones
    "Taste",
    "Taste bud",
    "Tear",
    "Tear duct",
    "Tear gland", // Lacrimal gland
    "Teeth", // Tooth
    "Telophase",
    "Temporal lobe",
    "Temporomandibular joint (TMJ)",
    "Tendon",
    "Testis", // Plural Testes
    "Testosterone",
    "Tetanus",
    "Thalamus",
    "Thigh",
    "Thigh bone", // Femur
    "Thirst",
    "Thoracic cavity",
    "Thoracic vertebrae",
    "Thorax", // Chest
    "Thrombin",
    "Thrombocyte", // Platelet
    "Thrombosis",
    "Thumb",
    "Thymus",
    "Thyroid gland",
    "Thyroid-stimulating hormone (TSH)",
    "Thyroxine", // T4
    "Tibia", // Shin bone
    "Tidal volume",
    "Tissue",
    "Toe",
    "Toenail",
    "Tongue",
    "Tonsil",
    "Tooth",
    "Torso",
    "Touch",
    "Toxin",
    "Trabecula",
    "Trachea", // Windpipe
    "Transverse colon",
    "Trapezius muscle",
    "Trauma",
    "Triceps",
    "Tricuspid valve",
    "Triglyceride",
    "Triiodothyronine", // T3
    "Trochanter",
    "Trypsin",
    "Tuberculosis",
    "Tubule",
    "Tumor",
    "Tympanic membrane", // Eardrum
    "Ulcer",
    "Ulna", // Forearm bone
    "Umbilical cord",
    "Umbilicus", // Navel
    "Unconscious",
    "Upper arm",
    "Upper back",
    "Upper leg", // Thigh
    "Urea",
    "Ureter",
    "Urethra",
    "Urinary bladder",
    "Urinary system",
    "Urinary tract",
    "Urination", // Micturition
    "Urine",
    "Urticaria", // Hives
    "Uterus", // Womb
    "Utricle",
    "Uvula",
    "Vaccine",
    "Vagina",
    "Vagus nerve",
    "Valve",
    "Varicose vein",
    "Vas deferens", // Ductus deferens
    "Vascular tissue",
    "Vasoconstriction",
    "Vasodilation",
    "Vein",
    "Vena cava",
    "Venom",
    "Ventral", // Anterior
    "Ventricle (heart)",
    "Venule",
    "Vermiform appendix", // Appendix
    "Vertebra", // Plural Vertebrae
    "Vertebral column", // Spine
    "Vestibular system",
    "Villus", // Plural Villi
    "Viscera",
    "Vision",
    "Visual cortex",
    "Vital capacity",
    "Vitamin",
    "Vitreous humour",
    "Vocal cord",
    "Voice box", // Larynx
    "Voluntary muscle", // Skeletal muscle
    "Vomer",
    "Vomiting",
    "Vulva",
    "Waist",
    "Wart",
    "Water",
    "Wax", // Cerumen
    "Wheezing",
    "White blood cell", // Leukocyte
    "White matter",
    "Windpipe", // Trachea
    "Wisdom tooth",
    "Womb", // Uterus
    "Wound",
    "Wrist",
    "Xiphoid process",
    "Yawn",
    "Yolk sac",
    "Zygomatic bone", // Cheekbone
    "Zygote",

    // Diseases and Disorders (Illustrative List - Needs Expansion)
    "Acromegaly",
    "Addison's disease",
    "AIDS (Acquired Immunodeficiency Syndrome)",
    "Alzheimer's disease",
    "Appendicitis",
    "Asperger syndrome",
    "Atherosclerosis",
    "Athlete's foot",
    "Autism spectrum disorder",
    "Autoimmune disease",
    "Beriberi",
    "Bipolar disorder",
    "Botulism",
    "Bronchitis",
    "Brucellosis",
    "Bubonic plague",
    "Bursitis",
    "Cancer",
    "Candidiasis",
    "Carpal tunnel syndrome",
    "Cataract",
    "Celiac disease",
    "Cerebral palsy",
    "Chickenpox",
    "Chlamydia",
    "Cholera",
    "Chronic fatigue syndrome",
    "Cirrhosis",
    "Common cold",
    "Congestive heart failure",
    "Conjunctivitis", // Pink eye
    "COVID-19",
    "Crohn's disease",
    "Cushing's syndrome",
    "Cystic fibrosis",
    "Dementia",
    "Dengue fever",
    "Depression",
    "Dermatitis",
    "Diphtheria",
    "Down syndrome",
    "Dysentery",
    "Dyslexia",
    "Ebola virus disease",
    "Eczema",
    "Emphysema",
    "Endometriosis",
    "Fibromyalgia",
    "Food poisoning",
    "Gallstones",
    "Gastritis",
    "Gastroenteritis",
    "Genital herpes",
    "Gigantism",
    "Glomerulonephritis",
    "Gonorrhea",
    "Grave's disease",
    "Guillain-Barré syndrome",
    "Hay fever",
    "Heart attack", // Myocardial infarction
    "Hemophilia",
    "Herpes simplex",
    "HIV (Human Immunodeficiency Virus)",
    "Hodgkin's lymphoma",
    "Huntington's disease",
    "Hyperparathyroidism",
    "Hypoparathyroidism",
    "Impetigo",
    "Irritable bowel syndrome (IBS)",
    "Kawasaki disease",
    "Klinefelter syndrome",
    "Kyphosis",
    "Lassa fever",
    "Legionnaires' disease",
    "Leprosy",
    "Leptospirosis",
    "Listeriosis",
    "Lupus",
    "Lyme disease",
    "Lymphoma",
    "Mad cow disease", // Bovine Spongiform Encephalopathy
    "Marfan syndrome",
    "Measles",
    "Melanoma",
    "Meningitis",
    "Mononucleosis",
    "Multiple sclerosis",
    "Mumps",
    "Muscular dystrophy",
    "Myasthenia gravis",
    "Myocardial infarction", // Heart attack
    "Narcolepsy",
    "Nephritis",
    "Obsessive-compulsive disorder (OCD)",
    "Otitis media", // Middle ear infection
    "Pancreatitis",
    "Panic disorder",
    "Parkinson's disease",
    "Pellagra",
    "Peptic ulcer disease",
    "Pericarditis",
    "Periodontitis",
    "Peritonitis",
    "Pertussis", // Whooping cough
    "Phenylketonuria (PKU)",
    "Pleurisy",
    "Poliomyelitis", // Polio
    "Polycystic ovary syndrome (PCOS)",
    "Porphyria",
    "Post-traumatic stress disorder (PTSD)",
    "Preeclampsia",
    "Prion disease",
    "Progeria",
    "Psoriasis",
    "Pulmonary embolism",
    "Q fever",
    "Raynaud's phenomenon",
    "Respiratory syncytial virus (RSV)",
    "Rheumatic fever",
    "Ringworm",
    "Rosacea",
    "Rotavirus infection",
    "Rubella", // German measles
    "Salmonellosis",
    "Sarcoidosis",
    "Scabies",
    "Scarlet fever",
    "Schistosomiasis",
    "Schizophrenia",
    "Sepsis",
    "Shingles",
    "Sickle cell anemia",
    "Sinusitis",
    "Sjögren's syndrome",
    "Smallpox",
    "Strep throat",
    "Syphilis",
    "Tay-Sachs disease",
    "Tendonitis",
    "Thalassemia",
    "Thrush", // Oral candidiasis
    "Tonsillitis",
    "Tourette syndrome",
    "Toxic shock syndrome",
    "Toxoplasmosis",
    "Trachoma",
    "Trichinosis",
    "Trypanosomiasis", // Sleeping sickness
    "Turner syndrome",
    "Typhoid fever",
    "Typhus",
    "Ulcerative colitis",
    "Urinary tract infection (UTI)",
    "Varicella", // Chickenpox
    "Vasculitis",
    "Vitiligo",
    "Von Willebrand disease",
    "West Nile virus infection",
    "Whooping cough", // Pertussis
    "Wilson's disease",
    "Yellow fever",
    "Zika virus disease",


    // Animals, Insects, Mammals, Nature
    "Aardvark",
    "Abdomen (insect)",
    "Abalone",
    "Acacia", // Plant
    "Accentor", // Bird
    "Adder", // Snake
    "Adelie penguin",
    "Agama", // Lizard
    "Agouti", // Rodent
    "Albatross",
    "Alder", // Plant
    "Algae",
    "Alligator",
    "Alpaca",
    "Amazon river dolphin",
    "Amberjack", // Fish
    "Ammonite", // Extinct mollusk
    "Amoeba", // Protist
    "Amphibian",
    "Anaconda", // Snake
    "Anchovy", // Fish
    "Anemone",
    "Angelfish",
    "Anglerfish",
    "Anhinga", // Bird
    "Anole", // Lizard
    "Ant",
    "Antarctic krill",
    "Anteater",
    "Antelope",
    "Antenna (insect/crustacean)",
    "Antler",
    "Anvil bone (ear)", // Incus
    "Ape",
    "Aphid", // Insect
    "Apteryx", // Kiwi bird
    "Aquatic",
    "Arachnid",
    "Arctic fox",
    "Arctic hare",
    "Arctic tern",
    "Argali", // Wild sheep
    "Armadillo",
    "Arrow worm",
    "Arthropod",
    "Aspen", // Plant
    "Ass", // Donkey
    "Auk", // Bird
    "Auricle (ear)", // Pinna
    "Avian", // Relating to birds
    "Avocet", // Bird
    "Axolotl", // Amphibian
    "Aye-aye", // Primate
    "Baboon",
    "Badger",
    "Bald eagle",
    "Baleen whale",
    "Bamboo", // Plant
    "Bandicoot", // Marsupial
    "Banteng", // Wild cattle
    "Barb", // Fish
    "Barbet", // Bird
    "Bark (tree)",
    "Barn owl",
    "Barnacle",
    "Barracuda",
    "Basilisk", // Lizard
    "Bass", // Fish
    "Bat",
    "Bear",
    "Bearded dragon", // Lizard
    "Beaver",
    "Bedbug",
    "Bee",
    "Bee-eater", // Bird
    "Beetle",
    "Beluga whale",
    "Bettong", // Marsupial
    "Bighorn sheep",
    "Bilby", // Marsupial
    "Binturong", // Mammal
    "Birch", // Plant
    "Bird",
    "Bird of paradise",
    "Bison",
    "Bittern", // Bird
    "Bivalve", // Mollusk
    "Black bear",
    "Blackbird",
    "Black panther", // Melanistic leopard/jaguar
    "Black rhino",
    "Black widow spider",
    "Blade (grass)",
    "Bleak", // Fish
    "Blenny", // Fish
    "Blesbok", // Antelope
    "Blister beetle",
    "Bloom (flower)",
    "Blowfish", // Pufferfish
    "Blue jay",
    "Blue whale",
    "Bluebird",
    "Bluefish",
    "Bluegill", // Fish
    "Boa constrictor",
    "Boar", // Wild pig
    "Bobcat",
    "Bobolink", // Bird
    "Bobwhite", // Quail
    "Boll weevil", // Insect
    "Bonobo", // Ape
    "Bony fish",
    "Booby", // Bird
    "Booklouse", // Insect
    "Bongo", // Antelope
    "Bowerbird",
    "Box turtle",
    "Brachiopod", // Marine invertebrate
    "Bramble", // Plant
    "Branch (tree)",
    "Brandt's vole",
    "Bream", // Fish
    "Brill", // Fish
    "Bristletail", // Insect
    "Brittle star", // Echinoderm
    "Broadbill", // Bird
    "Brocket", // Deer
    "Brown bear",
    "Bryophyte", // Moss, liverwort, hornwort
    "Bud (plant)",
    "Budgerigar", // Parakeet
    "Buffalo",
    "Bull",
    "Bullfrog",
    "Bullhead", // Catfish
    "Bullshark",
    "Bunting", // Bird
    "Burbot", // Fish
    "Bush", // Plant
    "Bush baby", // Galago
    "Bushbuck", // Antelope
    "Bushmaster", // Snake
    "Bustard", // Bird
    "Butcherbird",
    "Butterfly",
    "Butterflyfish",
    "Buzzard",
    "Caiman", // Reptile
    "Calf", // Young cow/elephant/whale etc.
    "Camel",
    "Canary",
    "Candiru", // Fish
    "Canvasback", // Duck
    "Cape buffalo",
    "Capercaillie", // Bird
    "Capuchin", // Monkey
    "Capybara",
    "Caracal", // Wild cat
    "Caracara", // Bird of prey
    "Carapace", // Shell
    "Cardinal",
    "Caribou", // Reindeer
    "Carp", // Fish
    "Carrier pigeon",
    "Cassowary", // Bird
    "Cat",
    "Caterpillar",
    "Catfish",
    "Cattle",
    "Cave lion", // Extinct
    "Cavy", // Rodent (guinea pig related)
    "Cedar", // Plant
    "Centipede",
    "Cephalopod", // Mollusk (squid, octopus)
    "Chaffinch", // Bird
    "Chameleon",
    "Chamois", // Goat-antelope
    "Char", // Fish
    "Cheetah",
    "Chick", // Young bird
    "Chickadee",
    "Chicken",
    "Chigger", // Mite larva
    "Chimpanzee",
    "Chinchilla",
    "Chipmunk",
    "Chital", // Deer
    "Chiton", // Mollusk
    "Chough", // Bird
    "Chub", // Fish
    "Cicada", // Insect
    "Cichlid", // Fish
    "Civet", // Mammal
    "Clam",
    "Claw",
    "Click beetle",
    "Clouded leopard",
    "Clownfish",
    "Clover", // Plant
    "Coati", // Mammal
    "Cobra",
    "Coccidian", // Protist
    "Cockatoo", // Bird
    "Cockle", // Mollusk
    "Cockroach",
    "Cod",
    "Codling", // Young cod
    "Coelacanth", // Fish
    "Colibri", // Hummingbird
    "Colobus", // Monkey
    "Colt", // Young horse
    "Comb jelly",
    "Comet moth",
    "Conch", // Mollusk
    "Condor",
    "Cone (plant)",
    "Conger", // Eel
    "Conifer", // Plant
    "Coot", // Bird
    "Copepod", // Crustacean
    "Copperhead", // Snake
    "Coral",
    "Coral snake",
    "Cormorant", // Bird
    "Corncrake", // Bird
    "Cotinga", // Bird
    "Cottonmouth", // Snake
    "Cottontail", // Rabbit
    "Coucal", // Bird
    "Cougar", // Puma, Mountain lion
    "Courser", // Bird
    "Cow",
    "Cowbird",
    "Cowrie", // Mollusk
    "Coyote",
    "Coypu", // Nutria
    "Crab",
    "Crab-eating macaque",
    "Crane",
    "Crane fly",
    "Crappie", // Fish
    "Crawdad", // Crayfish
    "Crayfish",
    "Creeper", // Bird
    "Cricket",
    "Crocodile",
    "Crossbill", // Bird
    "Crow",
    "Crustacean",
    "Cub", // Young bear/lion/tiger etc.
    "Cuckoo",
    "Cur", // Dog
    "Curassow", // Bird
    "Curlew",
    "Cuscus", // Marsupial
    "Cuttlefish",
    "Cycad", // Plant
    "Cypress", // Plant
    "Dab", // Fish
    "Dabchick", // Grebe
    "Dace", // Fish
    "Dachshund", // Dog
    "Dalmatian", // Dog
    "Damselfly", // Insect
    "Daphnia", // Crustacean
    "Darner", // Dragonfly
    "Darter", // Fish or Bird
    "Dassie", // Hyrax
    "Deer",
    "Desert",
    "Desert tortoise",
    "Desman", // Mammal
    "Devonian", // Geologic period
    "Dhole", // Wild dog
    "Dik-dik", // Antelope
    "Dingo",
    "Dinosaur",
    "Dipper", // Bird
    "Discus", // Fish
    "Diver", // Loon (bird)
    "Dodo", // Extinct bird
    "Doe", // Female deer/rabbit/goat
    "Dog",
    "Dogfish", // Shark
    "Dolphin",
    "Donkey",
    "Dorado", // Fish
    "Dormouse", // Rodent
    "Dotterel", // Bird
    "Dove",
    "Dragonfly",
    "Drake", // Male duck
    "Drill", // Primate
    "Dromedary", // Camel
    "Drongo", // Bird
    "Duck",
    "Duck-billed platypus", // Platypus
    "Dugong",
    "Duiker", // Antelope
    "Dunlin", // Bird
    "Dunnock", // Bird
    "Eagle",
    "Eagle ray",
    "Earthworm",
    "Earwig", // Insect
    "Echidna",
    "Echinoderm", // Starfish, sea urchin
    "Ecology", // Study of ecosystems
    "Ecosystem",
    "Eel",
    "Eft", // Newt
    "Egg",
    "Egret",
    "Eider", // Duck
    "Eland", // Antelope
    "Electric eel",
    "Electric ray",
    "Elephant",
    "Elephant seal",
    "Elephant shrew",
    "Elk", // Wapiti
    "Emu",
    "Environment",
    "Ermine", // Stoat
    "Estuary",
    "Euphausiid", // Krill
    "Evolution", // Biological process
    "Extremophile", // Organism in extreme conditions
    "Eyass", // Young hawk
    "Falcon",
    "Fallow deer",
    "Fang",
    "Fauna",
    "Fawn", // Young deer
    "Feather",
    "Feline", // Cat
    "Fennec fox",
    "Fer-de-lance", // Snake
    "Fern", // Plant
    "Ferret",
    "Fiddler crab",
    "Fieldfare", // Bird
    "Field mouse",
    "Fig wasp",
    "Filefish",
    "Filly", // Young female horse
    "Finch",
    "Fin whale",
    "Fir", // Plant
    "Fire ant",
    "Firecrest", // Bird
    "Firefly",
    "Fish",
    "Fisher", // Mammal
    "Fishing cat",
    "Flamingo",
    "Flatfish",
    "Flea",
    "Flicker", // Woodpecker
    "Flounder",
    "Flower", // Plant part
    "Fly",
    "Flycatcher", // Bird
    "Flying fish",
    "Flying fox", // Bat
    "Flying squirrel",
    "Foal", // Young horse
    "Fog", // Weather
    "Food chain",
    "Food web",
    "Forest",
    "Fossa", // Mammal
    "Fossil",
    "Fowl", // Bird (chicken, turkey)
    "Fox",
    "Foxtail", // Plant
    "Frigatebird",
    "Frilled lizard",
    "Frog",
    "Frogmouth", // Bird
    "Fruit", // Plant part
    "Fruit bat",
    "Fruit fly",
    "Fulmar", // Bird
    "Fungus", // Plural Fungi
    "Fur",
    "Fur seal",
    "Gadwall", // Duck
    "Galago", // Bush baby
    "Galah", // Bird
    "Gall", // Plant growth
    "Gallinule", // Bird
    "Gannet", // Bird
    "Gar", // Fish
    "Garden spider",
    "Garganey", // Duck
    "Gaur", // Wild cattle
    "Gazelle",
    "Gecko",
    "Gelada", // Primate
    "Gemsbok", // Oryx
    "Genet", // Mammal
    "Gerbil",
    "Gerenuk", // Antelope
    "Gharial", // Reptile
    "Ghost crab",
    "Giant panda",
    "Giant salamander",
    "Giant schnauzer", // Dog
    "Giant squid",
    "Gibbon",
    "Gila monster", // Lizard
    "Gill", // Respiratory organ
    "Giraffe",
    "Glass lizard",
    "Glass snake", // Legless lizard
    "Glider", // Marsupial
    "Glowworm", // Beetle larva
    "Gnat",
    "Gnu", // Wildebeest
    "Goat",
    "Goatfish",
    "Godwit", // Bird
    "Golden eagle",
    "Golden hamster",
    "Golden retriever", // Dog
    "Goldeneye", // Duck
    "Goldfinch",
    "Goldfish",
    "Goosander", // Merganser
    "Goose",
    "Gopher",
    "Goral", // Goat-antelope
    "Gorilla",
    "Goshawk", // Bird
    "Gosling", // Young goose
    "Gourami", // Fish
    "Grass", // Plant
    "Grasshopper",
    "Gray whale",
    "Grayling", // Fish
    "Great auk", // Extinct bird
    "Great Dane", // Dog
    "Great white shark",
    "Grebe", // Bird
    "Greenfinch", // Bird
    "Greenland shark",
    "Greenling", // Fish
    "Greenshank", // Bird
    "Greyhound", // Dog
    "Grison", // Mammal
    "Grizzly bear",
    "Grosbeak", // Bird
    "Ground beetle",
    "Groundhog", // Woodchuck
    "Ground squirrel",
    "Grouper", // Fish
    "Grouse",
    "Guan", // Bird
    "Guanaco",
    "Guillemot", // Bird
    "Guinea fowl",
    "Guinea pig",
    "Gull",
    "Guppy",
    "Gurnard", // Fish
    "Gyrfalcon", // Bird
    "Habitat",
    "Haddock",
    "Hagfish",
    "Hake", // Fish
    "Halibut",
    "Hamadryas baboon",
    "Hammerhead shark",
    "Hamster",
    "Harbor seal",
    "Hardhead", // Fish
    "Hare",
    "Harlequin duck",
    "Harp seal",
    "Harpy eagle",
    "Harrier", // Bird
    "Hart", // Male deer
    "Hartebeest", // Antelope
    "Harvest mouse",
    "Harvestman", // Arachnid
    "Hatchetfish",
    "Hawk",
    "Hedgehog",
    "Helicopter damselfly",
    "Hellbender", // Salamander
    "Hen", // Female bird
    "Hermit crab",
    "Heron",
    "Herring",
    "Hind", // Female deer
    "Hippopotamus",
    "Hoatzin", // Bird
    "Hog", // Pig
    "Hoki", // Fish
    "Honey badger",
    "Honeybee",
    "Honeyeater", // Bird
    "Hoopoe", // Bird
    "Hornbill", // Bird
    "Horned lizard",
    "Horned viper",
    "Hornet",
    "Horse",
    "Horsefly",
    "Hound", // Dog
    "Housefly",
    "Hoverfly",
    "Howler monkey",
    "Human",
    "Humboldt squid",
    "Hummingbird",
    "Humpback whale",
    "Hutia", // Rodent
    "Hyacinth macaw",
    "Hydra", // Cnidarian
    "Hydrozoa", // Class including hydras
    "Hyena",
    "Hyrax", // Dassie
    "Ibex", // Wild goat
    "Ibis",
    "Icefish",
    "Ichthyosaur", // Extinct marine reptile
    "Iguana",
    "Iguanodon", // Dinosaur
    "Impala",
    "Inchworm", // Moth larva
    "Indigo bunting", // Bird
    "Indri", // Primate
    "Insect",
    "Insectivore", // Animal that eats insects
    "Invertebrate",
    "Isopod", // Crustacean
    "Ivory", // Material from tusks
    "Jacamar", // Bird
    "Jacana", // Bird
    "Jackal",
    "Jackdaw", // Bird
    "Jackrabbit", // Hare
    "Jaeger", // Bird
    "Jaguar",
    "Jaguarundi", // Wild cat
    "Japanese beetle",
    "Japanese macaque",
    "Jay",
    "Jellyfish",
    "Jerboa", // Rodent
    "Jewel beetle",
    "Jewelfish",
    "John Dory", // Fish
    "Jungle", // Habitat
    "Junglefowl", // Bird
    "Junco", // Bird
    "Juniper", // Plant
    "Kagu", // Bird
    "Kakapo", // Bird
    "Kangaroo",
    "Kangaroo rat",
    "Katydid", // Insect
    "Kelp", // Seaweed
    "Kestrel", // Bird
    "Kid", // Young goat
    "Killdeer", // Bird
    "Killer whale", // Orca
    "Killifish",
    "Kin", // Relative
    "King cobra",
    "King crab",
    "King eider", // Duck
    "King penguin",
    "Kingbird",
    "Kingfisher",
    "Kinglet", // Bird
    "Kingsnake",
    "Kinkajou", // Mammal
    "Kite", // Bird
    "Kit fox",
    "Kitten", // Young cat
    "Kittiwake", // Bird
    "Kiwi", // Bird
    "Knifefish",
    "Knight anole", // Lizard
    "Knot", // Bird
    "Koala",
    "Kobus", // Waterbuck genus
    "Koi", // Fish
    "Kokanee", // Salmon
    "Komodo dragon",
    "Kookaburra",
    "Kouprey", // Wild cattle
    "Krill", // Crustacean
    "Kudu", // Antelope
    "Kultarr", // Marsupial
    "Labrador retriever", // Dog
    "Lacewing", // Insect
    "Ladybug", // Ladybird beetle
    "Lagomorph", // Rabbit, hare, pika
    "Lake", // Habitat
    "Lake trout",
    "Lamb", // Young sheep
    "Lamprey",
    "Lancelet", // Chordate
    "Land snail",
    "Langur", // Monkey
    "Lanternfish",
    "Lapwing", // Bird
    "Larch", // Plant
    "Lark",
    "Larva", // Immature stage
    "Lawn", // Habitat
    "Leaf", // Plant part
    "Leaf beetle",
    "Leafhopper", // Insect
    "Leaf insect",
    "Leaf-cutter ant",
    "Lechwe", // Antelope
    "Leech",
    "Leek moth",
    "Left atrium", // Heart part
    "Left ventricle", // Heart part
    "Leg", // Appendage
    "Legless lizard",
    "Leipoa", // Malleefowl
    "Lemming",
    "Lemur",
    "Leopard",
    "Leopard cat",
    "Leopard frog",
    "Leopard seal",
    "Leveret", // Young hare
    "Lice", // Plural of Louse
    "Lichen", // Composite organism
    "Liger",
    "Light", // Energy form
    "Lightning bug", // Firefly
    "Limpet", // Mollusk
    "Line", // Fish type
    "Ling", // Fish
    "Linnet", // Bird
    "Lion",
    "Lionfish",
    "Litter", // Group of young animals
    "Little blue penguin",
    "Liver fluke", // Parasite
    "Lizard",
    "Llama",
    "Loach", // Fish
    "Lobster",
    "Locust",
    "Lodgepole pine", // Plant
    "Loggerhead turtle",
    "Long-eared owl",
    "Longhorn beetle",
    "Longspur", // Bird
    "Loon", // Bird
    "Lorikeet", // Bird
    "Loris",
    "Louse", // Singular of Lice
    "Lovebird",
    "Lowland gorilla",
    "Luciferin", // Light-emitting compound
    "Lugworm",
    "Lumpsucker", // Fish
    "Lungfish",
    "Lupine", // Plant
    "Lurcher", // Dog type
    "Lychee", // Fruit/Plant
    "Lynx",
    "Lyrebird",
    "Macaque", // Monkey
    "Macaw",
    "Mackerel",
    "Madagascar", // Location relevant to many species
    "Maggot", // Fly larva
    "Magpie",
    "Mahi-mahi", // Fish
    "Maidenhair tree", // Ginkgo
    "Maize", // Corn plant
    "Malachite", // Mineral / Butterfly name
    "Malaria mosquito", // Anopheles
    "Mallard",
    "Malleefowl", // Bird
    "Mamba", // Snake
    "Mammal",
    "Mammoth", // Extinct elephant relative
    "Manakin", // Bird
    "Manatee",
    "Mandarin duck",
    "Mandrill",
    "Mangabey", // Monkey
    "Mangrove", // Plant/Habitat
    "Manta ray",
    "Mantis",
    "Mantis shrimp",
    "Manx cat",
    "Maple", // Plant
    "Mara", // Rodent
    "Marabou stork",
    "Marbled cat",
    "Mare", // Female horse
    "Margay", // Wild cat
    "Marine", // Relating to the sea
    "Marlin",
    "Marmoset",
    "Marmot",
    "Mars", // Planet (context?)
    "Marsh", // Habitat
    "Marsupial",
    "Marsupial mole",
    "Marten",
    "Martin", // Bird
    "Masked booby", // Bird
    "Mastiff", // Dog
    "Mastodon", // Extinct elephant relative
    "Mayfly", // Insect
    "Meadow", // Habitat
    "Meadowlark", // Bird
    "Mealworm", // Beetle larva
    "Meerkat",
    "Megalodon", // Extinct shark
    "Megapode", // Bird group
    "Merganser", // Duck
    "Merlin", // Falcon
    "Metamorphosis", // Biological process
    "Midge", // Insect
    "Migration", // Animal movement
    "Milkfish",
    "Milksnake",
    "Millipede",
    "Mink",
    "Minnow", // Fish
    "Mite", // Arachnid
    "Moa", // Extinct bird
    "Mockingbird",
    "Mole",
    "Mole cricket",
    "Mole rat",
    "Mollusk",
    "Molly", // Fish
    "Monarch butterfly",
    "Mongoose",
    "Monitor lizard",
    "Monkey",
    "Monkfish",
    "Monkshood", // Plant
    "Monotreme", // Egg-laying mammal
    "Moorhen", // Bird
    "Moose",
    "Moray eel",
    "Mosquito",
    "Moss", // Plant
    "Moth",
    "Motmot", // Bird
    "Mouflon", // Wild sheep
    "Mountain", // Habitat
    "Mountain goat",
    "Mountain gorilla",
    "Mountain lion", // Puma
    "Mouse",
    "Mousebird",
    "Mouse-deer", // Chevrotain
    "Mudfish",
    "Mudpuppy", // Salamander
    "Mudskipper", // Fish
    "Mud dauber", // Wasp
    "Mule",
    "Mule deer",
    "Mullet", // Fish
    "Muntjac", // Deer
    "Murre", // Bird
    "Musk deer",
    "Musk ox",
    "Muskellunge", // Fish
    "Muskrat",
    "Mussel",
    "Mustang", // Horse
    "Mustelid", // Weasel family
    "Myna", // Bird
    "Myriapod", // Centipede/Millipede group
    "Mysis", // Shrimp
    "Nabarlek", // Marsupial
    "Nag", // Old horse
    "Naked mole-rat",
    "Narwhal",
    "Nase", // Fish
    "Nature",
    "Nautilus", // Mollusk
    "Needlefish",
    "Nematode", // Roundworm
    "Neon tetra", // Fish
    "Nereis", // Worm
    "Nest", // Animal home
    "Nettle", // Plant
    "Newfoundland", // Dog
    "Newt",
    "Nightingale",
    "Nightjar", // Bird
    "Nile crocodile",
    "Nile perch", // Fish
    "Nilgai", // Antelope
    "Nocturnal", // Active at night
    "Noddy", // Bird
    "Norway rat", // Brown rat
    "Nose-horned viper",
    "Nothura", // Bird
    "Nudibranch", // Sea slug
    "Numbat", // Marsupial
    "Nurse shark",
    "Nut", // Fruit type
    "Nutcracker", // Bird
    "Nuthatch", // Bird
    "Nutria", // Coypu
    "Nyala", // Antelope
    "Oak", // Plant
    "Oarfish",
    "Oasis", // Habitat
    "Oat", // Plant
    "Ocelot", // Wild cat
    "Octopus",
    "Okapi",
    "Oldsquaw", // Long-tailed duck
    "Olingo", // Mammal
    "Olive", // Plant/Fruit
    "Olm", // Amphibian
    "Onager", // Wild ass
    "Opossum",
    "Orangutan",
    "Orb weaver", // Spider
    "Orca", // Killer whale
    "Orchid", // Plant
    "Organism",
    "Oriole", // Bird
    "Oropendola", // Bird
    "Oryx", // Antelope
    "Osprey", // Bird
    "Ostracod", // Crustacean
    "Ostrich",
    "Otter",
    "Ovenbird",
    "Oviparous", // Egg-laying
    "Ovoviviparous", // Eggs hatch internally
    "Owl",
    "Owlfly", // Insect
    "Ox",
    "Oxpecker", // Bird
    "Oyster",
    "Oystercatcher", // Bird
    "Paca", // Rodent
    "Pachycephalosaurus", // Dinosaur
    "Paddlefish",
    "Pademelon", // Marsupial
    "Paedophryne amauensis", // Smallest frog
    "Pagoda", // Structure (context?)
    "Pallas's cat",
    "Palm tree", // Plant
    "Pampas cat",
    "Panda",
    "Pangolin", // Mammal
    "Panther", // Leopard/Jaguar or Cougar
    "Papaya", // Plant/Fruit
    "Paper wasp",
    "Papillon", // Dog / Butterfly
    "Parakeet",
    "Parasaurolophus", // Dinosaur
    "Parrot",
    "Parrotfish",
    "Partridge",
    "Passenger pigeon", // Extinct bird
    "Passerine", // Perching bird
    "Patas monkey",
    "Paua", // Abalone
    "Pea", // Plant/Seed
    "Peacock", // Male peafowl
    "Peafowl",
    "Pear", // Plant/Fruit
    "Peccary", // Mammal
    "Pelican",
    "Penguin",
    "Perch", // Fish
    "Peregrine falcon",
    "Perentie", // Lizard
    "Periwinkle", // Mollusk / Plant
    "Petrel", // Bird
    "Phainopepla", // Bird
    "Phalarope", // Bird
    "Pheasant",
    "Phoebe", // Bird
    "Photosynthesis", // Biological process
    "Phytoplankton", // Microscopic plants
    "Pica", // Bird genus (magpies) / Eating disorder
    "Pichi", // Armadillo
    "Pickerel", // Fish
    "Pig",
    "Pigeon",
    "Pika", // Mammal
    "Pike", // Fish
    "Pikeperch", // Zander
    "Pilchard", // Sardine
    "Pilot whale",
    "Pine", // Plant
    "Pine marten",
    "Pine siskin", // Bird
    "Pink fairy armadillo",
    "Pinniped", // Seal, sea lion, walrus
    "Pintail", // Duck
    "Pipefish",
    "Pipit", // Bird
    "Piranha", // Fish
    "Pistil", // Plant part
    "Pit viper", // Snake
    "Pitta", // Bird
    "Plains", // Habitat
    "Planarian", // Flatworm
    "Plankton",
    "Plant",
    "Plantain", // Plant / Bird name
    "Planula", // Larva
    "Platypus",
    "Plesiosaur", // Extinct marine reptile
    "Plover", // Bird
    "Plum", // Plant/Fruit
    "Plume", // Feather
    "Poacher", // Fish
    "Pochard", // Duck
    "Pocket gopher",
    "Pocket mouse",
    "Pod", // Group of whales/dolphins / Seed container
    "Pointer", // Dog
    "Poison dart frog",
    "Polar bear",
    "Polecat", // Mammal
    "Pollan", // Fish
    "Pollard", // Tree pruning style
    "Pollination", // Plant reproduction process
    "Polliwog", // Tadpole
    "Polyp", // Cnidarian form
    "Pomeranian", // Dog
    "Pompano", // Fish
    "Pond", // Habitat
    "Pond skater", // Insect
    "Pony",
    "Poodle", // Dog
    "Poorwill", // Bird
    "Poplar", // Plant
    "Poppy", // Plant
    "Porcupine",
    "Porcupinefish",
    "Porpoise",
    "Portuguese man-of-war", // Siphonophore
    "Possum", // Opossum
    "Potato", // Plant/Tuber
    "Potoroo", // Marsupial
    "Potto", // Primate
    "Poultry", // Domestic birds
    "Pound", // Unit of mass / Animal shelter
    "Prairie", // Habitat
    "Prairie chicken",
    "Prairie dog",
    "Prawn",
    "Praying mantis", // Mantis
    "Primate",
    "Prion", // Bird / Protein particle
    "Proboscis", // Nose/Snout
    "Proboscis monkey",
    "Pronghorn", // Mammal
    "Protozoa", // Single-celled eukaryotes
    "Ptarmigan", // Bird
    "Pterodactyl", // Pterosaur
    "Pterosaur", // Extinct flying reptile
    "Pufferfish",
    "Puffin",
    "Pug", // Dog
    "Pudu", // Deer
    "Puma", // Cougar
    "Pupa", // Insect stage
    "Purple finch",
    "Purple martin", // Bird
    "Puss moth",
    "Pygmy marmoset",
    "Pygmy possum",
    "Python",
    "Quagga", // Extinct zebra subspecies
    "Quail",
    "Quaker parrot", // Monk parakeet
    "Queen ant",
    "Queen bee",
    "Queen snake",
    "Quelea", // Bird
    "Quetzal", // Bird
    "Quokka",
    "Quoll", // Marsupial
    "Rabbit",
    "Raccoon",
    "Racer", // Snake
    "Racerunner", // Lizard
    "Ragworm",
    "Rail", // Bird
    "Rainbow lorikeet",
    "Rainbow trout",
    "Rainforest", // Habitat
    "Ram", // Male sheep
    "Raptor", // Bird of prey / Dinosaur type
    "Raspberry", // Plant/Fruit
    "Rat",
    "Ratfish", // Chimaera
    "Rattail", // Fish
    "Rattlesnake",
    "Raven",
    "Ray", // Fish (Manta, Stingray, Electric)
    "Razor clam",
    "Red deer",
    "Red drum", // Fish
    "Red fox",
    "Red kite", // Bird
    "Red panda",
    "Red poll", // Bird
    "Red squirrel",
    "Red wolf",
    "Redstart", // Bird
    "Redwing", // Bird
    "Reed", // Plant
    "Reedbuck", // Antelope
    "Reef", // Habitat (coral reef)
    "Regent bowerbird",
    "Reindeer", // Caribou
    "Remora", // Fish
    "Reptile",
    "Rhea", // Bird
    "Rhesus macaque", // Monkey
    "Rhinoceros",
    "Rhinoceros beetle",
    "Rhizome", // Plant stem
    "River", // Habitat
    "River dolphin",
    "Roadrunner",
    "Roach", // Fish / Cockroach
    "Roan antelope",
    "Robin",
    "Rock bass", // Fish
    "Rock beauty", // Fish
    "Rock crab",
    "Rock dove", // Pigeon
    "Rock hyrax", // Dassie
    "Rock ptarmigan",
    "Rockfish",
    "Rodent",
    "Roe", // Fish eggs / Deer type
    "Roe deer",
    "Roller", // Bird
    "Rook", // Bird
    "Rooster", // Male chicken
    "Root", // Plant part
    "Rose", // Plant/Flower
    "Roseate spoonbill", // Bird
    "Rosefinch",
    "Rotifer", // Microscopic animal
    "Rough-legged hawk",
    "Roundworm", // Nematode
    "Rove beetle",
    "Royal penguin",
    "Rubber boa", // Snake
    "Ruby-throated hummingbird",
    "Rudd", // Fish
    "Ruff", // Bird
    "Ruffed grouse",
    "Rufous hummingbird",
    "Russian blue", // Cat breed
    "Sable", // Mammal
    "Sable antelope",
    "Sac-winged bat",
    "Sage", // Plant
    "Sage grouse", // Bird
    "Sago palm", // Plant (cycad)
    "Sailfish",
    "Saiga antelope",
    "Saki", // Monkey
    "Salal", // Plant
    "Salamander",
    "Salmon",
    "Salt", // Chemical compound
    "Saltwater crocodile",
    "Sambar", // Deer
    "Sand cat",
    "Sand dollar",
    "Sand lizard",
    "Sand martin", // Bank swallow
    "Sandpiper",
    "Sandworm",
    "Sapsucker", // Woodpecker
    "Sardine",
    "Sargassum fish",
    "Sarus crane",
    "Satin bowerbird",
    "Sauropod", // Dinosaur group
    "Savanna", // Habitat
    "Sawfish",
    "Sawfly", // Insect
    "Scad", // Fish
    "Scale (fish/reptile)",
    "Scaled quail",
    "Scallop",
    "Scarab beetle",
    "Scarlet ibis",
    "Scarlet macaw",
    "Scaup", // Duck
    "Scent gland",
    "Schneider's skink", // Lizard
    "Scimitar oryx", // Extinct in wild
    "Scissor-tailed flycatcher",
    "Scolopendra", // Centipede genus
    "Scoter", // Duck
    "Scorpion",
    "Scorpion fly",
    "Scorpionfish",
    "Screamer", // Bird
    "Scrub", // Habitat / Plant type
    "Scrub jay",
    "Sculpin", // Fish
    "Scythebill", // Bird
    "Sea", // Ocean
    "Sea anemone",
    "Sea bass",
    "Sea bear", // Fur seal
    "Sea bream", // Fish
    "Sea cow", // Manatee/Dugong
    "Sea cucumber", // Echinoderm
    "Sea dragon", // Fish
    "Sea eagle",
    "Sea fan", // Coral
    "Sea gooseberry", // Comb jelly
    "Sea hare", // Sea slug
    "Sea horse", // Seahorse
    "Sea level", // Measurement
    "Sea lily", // Crinoid
    "Sea lion",
    "Sea otter",
    "Sea pen", // Coral relative
    "Sea raven", // Fish
    "Sea robin", // Fish
    "Sea slug",
    "Sea snake",
    "Sea squirt", // Tunicate
    "Sea star", // Starfish
    "Sea turtle",
    "Sea urchin",
    "Seabird",
    "Seadragon", // Fish related to seahorse
    "Seagrass", // Plant
    "Seagull", // Gull
    "Seahorse",
    "Seal",
    "Season", // Time period
    "Seaweed", // Algae
    "Secretary bird",
    "Sedge", // Plant
    "Seed", // Plant part
    "Seedling", // Young plant
    "Sei whale",
    "Selachimorpha", // Sharks
    "Senegal bushbaby", // Galago
    "Sensitive plant", // Mimosa pudica
    "Seriema", // Bird
    "Serval", // Wild cat
    "Servaline genet",
    "Sessile", // Attached, non-moving
    "Setophaga", // Warbler genus
    "Setter", // Dog
    "Seven-spot ladybird",
    "Sex", // Biological characteristic
    "Seychelles giant tortoise",
    "Shad", // Fish
    "Shag", // Cormorant
    "Shark",
    "Shearwater", // Bird
    "Sheathbill", // Bird
    "Sheep",
    "Shelduck", // Duck
    "Shell", // Outer covering
    "Shellfish", // Aquatic invertebrate
    "Shiba Inu", // Dog
    "Shima enaga", // Bird subspecies
    "Shiner", // Fish
    "Shipworm", // Mollusk
    "Shoveler", // Duck
    "Shrew",
    "Shrike", // Bird
    "Shrimp",
    "Siamese cat",
    "Siberian husky", // Dog
    "Siberian tiger",
    "Sifaka", // Lemur
    "Silkworm", // Moth larva
    "Silverfish", // Insect
    "Silverside", // Fish
    "Singing", // Bird vocalization
    "Siphonophore", // Marine colony
    "Siren", // Amphibian
    "Sirkeer malkoha", // Bird
    "Siskin", // Bird
    "Sitatunga", // Antelope
    "Skate", // Fish
    "Skimmer", // Bird / Insect
    "Skink", // Lizard
    "Skipper", // Butterfly / Fish
    "Skua", // Bird
    "Skunk",
    "Skylark", // Bird
    "Sleeper", // Fish
    "Slime mold", // Protist
    "Sloth",
    "Slow worm", // Legless lizard
    "Slug", // Mollusk
    "Smelt", // Fish
    "Smew", // Duck
    "Snail",
    "Snake",
    "Snakebird", // Anhinga
    "Snakefly", // Insect
    "Snapper", // Fish
    "Snapping turtle",
    "Snipe", // Bird
    "Snout", // Nose
    "Snow", // Precipitation
    "Snow bunting", // Bird
    "Snow goose",
    "Snow leopard",
    "Snowshoe hare",
    "Snowy owl",
    "Snowy plover", // Bird
    "Soil", // Earth layer
    "Solenodon", // Mammal
    "Sole", // Fish
    "Solitaire", // Bird
    "Somali wild ass",
    "Songbird",
    "Sora", // Bird
    "Sotalia", // Dolphin genus
    "South American coati",
    "Sow", // Female pig
    "Spadefoot toad",
    "Spaniel", // Dog
    "Spanish mackerel",
    "Sparrow",
    "Sparrowhawk",
    "Spatula", // Spoonbill genus / Tool
    "Spawn", // Eggs/young of aquatic animals
    "Spectacled bear",
    "Spectacled caiman",
    "Sperm whale",
    "Spider",
    "Spider monkey",
    "Spider wasp",
    "Spiderhunter", // Bird
    "Spike", // Plant structure / Fish fin ray
    "Spinebill", // Bird
    "Spinner dolphin",
    "Spinner shark",
    "Spinulosida", // Starfish order
    "Spiny dogfish", // Shark
    "Spiny lobster",
    "Spiny mouse",
    "Spiracle", // Respiratory opening
    "Spire", // Shell part
    "Spittlebug", // Insect
    "Splash tetra", // Fish
    "Sponge",
    "Spoonbill", // Bird
    "Spore", // Reproductive unit
    "Sport", // Genetic mutation / Activity
    "Spotted bass",
    "Spotted crake", // Bird
    "Spotted cuscus",
    "Spotted dolphin",
    "Spotted eagle ray",
    "Spotted hyena",
    "Spotted sandpiper",
    "Sprat", // Fish
    "Spring", // Season / Water source
    "Springbok", // Antelope
    "Springhaas", // Mammal
    "Springtail", // Arthropod
    "Spruce", // Plant
    "Spur", // Bony projection
    "Spurfowl", // Bird
    "Squacco heron",
    "Squamate", // Scaled reptile
    "Squash bug", // Insect
    "Squid",
    "Squirrel",
    "Squirrelfish",
    "Stag", // Male deer
    "Stag beetle",
    "Stallion", // Male horse
    "Stamen", // Plant part
    "Starfish",
    "Star-nosed mole",
    "Starlet", // Fish
    "Starling",
    "Steelhead", // Trout
    "Steenbok", // Antelope
    "Stegosaurus", // Dinosaur
    "Stem", // Plant part
    "Stent", // Medical device / Organism name?
    "Sterlet", // Sturgeon
    "Stick insect",
    "Stickleback", // Fish
    "Stilt", // Bird
    "Sting", // Injury / Insect part
    "Stingray",
    "Stink bug",
    "Stinkpot", // Musk turtle
    "Stoat", // Ermine
    "Stone curlew", // Bird
    "Stonechat", // Bird
    "Stonefish",
    "Stonefly", // Insect
    "Stork",
    "Storm petrel", // Bird
    "Strawberry", // Plant/Fruit
    "Stream", // Water body
    "Stromatolite", // Fossil structure
    "Struthio", // Ostrich genus
    "Stud", // Male animal for breeding
    "Sturgeon",
    "Sugar glider", // Marsupial
    "Sulphur-crested cockatoo",
    "Sumatran orangutan",
    "Sumatran rhinoceros",
    "Sumatran tiger",
    "Summer", // Season
    "Sun", // Star
    "Sun bear",
    "Sunbeam snake",
    "Sunbittern", // Bird
    "Sunbird",
    "Sunfish",
    "Sunflower", // Plant
    "Suni", // Antelope
    "Surf scoter", // Duck
    "Surgeonfish",
    "Suricate", // Meerkat
    "Suslik", // Ground squirrel
    "Swallow", // Bird
    "Swallowtail butterfly",
    "Swamp", // Habitat
    "Swamphen", // Bird
    "Swan",
    "Sweat bee",
    "Swift", // Bird
    "Swift fox",
    "Swiftlet", // Bird
    "Swordfish",
    "Sycamore", // Plant
    "Symphylan", // Arthropod
    "Tadpole", // Frog larva
    "Taiga", // Habitat
    "Tail", // Appendage
    "Taipan", // Snake
    "Takahe", // Bird
    "Takin",
    "Talapoin", // Monkey
    "Tamandua", // Anteater
    "Tamarin", // Monkey
    "Tanager", // Bird
    "Tapanuli orangutan",
    "Tapeworm", // Parasite
    "Tapir",
    "Tarantula",
    "Tarpan", // Extinct wild horse
    "Tarsier",
    "Taruca", // Deer
    "Tasmanian devil",
    "Tasmanian tiger", // Thylacine (extinct)
    "Tattler", // Bird
    "Tayra", // Mammal
    "Teal", // Duck
    "Tegu", // Lizard
    "Teledu", // Badger
    "Tench", // Fish
    "Tenebrionid beetle",
    "Tenrec", // Mammal
    "Termite",
    "Tern",
    "Terrapin", // Turtle
    "Terrier", // Dog
    "Tetanocera", // Fly genus
    "Tetra", // Fish
    "Texas blind salamander",
    "Thicket", // Habitat
    "Thorn", // Plant part
    "Thorny devil", // Lizard
    "Thrasher", // Bird
    "Threadfin", // Fish
    "Three-toed sloth",
    "Thrip", // Insect
    "Thrush", // Bird
    "Thylacine", // Tasmanian tiger (extinct)
    "Tibetan antelope",
    "Tibetan fox",
    "Tibetan mastiff", // Dog
    "Tick", // Arachnid
    "Tiger",
    "Tiger beetle",
    "Tiger moth",
    "Tiger salamander",
    "Tiger shark",
    "Tiglon", // Hybrid big cat
    "Tigon", // Hybrid big cat
    "Tilefish",
    "Timber rattlesnake",
    "Timber wolf", // Gray wolf
    "Tinamou", // Bird
    "Titi", // Monkey
    "Titmouse", // Bird
    "Toad",
    "Toadfish",
    "Tody", // Bird
    "Tokeh gecko",
    "Tomtit", // Bird
    "Topi", // Antelope
    "Topminnow", // Fish
    "Torpedo", // Electric ray
    "Tortoise",
    "Toucan",
    "Toucanet", // Small toucan
    "Towel", // Material (context?)
    "Toxin", // Poisonous substance
    "Tragopan", // Bird
    "Transpiration", // Plant process
    "Tree", // Plant
    "Tree frog",
    "Tree kangaroo",
    "Tree pipit", // Bird
    "Tree shrew",
    "Tree snake",
    "Tree squirrel",
    "Tree swallow",
    "Treecreeper", // Bird
    "Trevally", // Fish
    "Triggerfish",
    "Trilobite", // Extinct arthropod
    "Trillium", // Plant
    "Trogon", // Bird
    "Tropicbird",
    "Trout",
    "Trumpeter", // Bird
    "Trumpeter swan",
    "Trunk", // Elephant's nose / Tree part
    "Tsetse fly",
    "Tuatara", // Reptile
    "Tuber", // Plant part
    "Tsetse fly", // Insect
    "Tuna",
    "Tundra", // Habitat
    "Tundra swan",
    "Tunicate", // Sea squirt
    "Turbot", // Fish
    "Turkey",
    "Turnstone", // Bird
    "Turtle",
    "Turtle dove",
    "Tusk", // Tooth
    "Tufted deer",
    "Tufted titmouse",
    "Tui", // Bird
    "Tuna", // Fish
    "Tundra", // Biome
    "Tunicate", // Marine invertebrate
    "Turbot", // Fish
    "Turkey", // Bird
    "Turnip", // Plant
    "Turnstone", // Bird
    "Turtle", // Reptile
    "Turtle dove", // Bird
    "Tusk", // Elongated tooth
    "Tutle", // Likely typo for Turtle
    "Tussock moth",
    "Twig", // Small tree branch
    "Two-toed sloth",
    "Tyrannosaurus rex", // Dinosaur
    "Uakari", // Monkey
    "Uaru", // Fish
    "Uguisu", // Japanese bush warbler
    "Uinta ground squirrel",
    "Uirapuru", // Bird
    "Urial", // Wild sheep
    "Uromastyx", // Lizard
    "Urutu", // Snake
    "Umbrellabird",
    "Undergrowth", // Vegetation layer
    "Ungulate", // Hoofed mammal
    "Upland sandpiper",
    "Ural owl",
    "Uranoscopus", // Stargazer fish
    "Urchin", // Sea urchin
    "Urial", // Wild sheep
    "Uromastyx", // Lizard
    "Urson", // Porcupine
    "Urubu", // Vulture
    "Urutu", // Snake
    "Utopia", // Ideal place (context?)
    "Vampire bat",
    "Vampire finch",
    "Vampire squid",
    "Vaquita", // Porpoise
    "Variable hawk",
    "Variable oystercatcher",
    "Vegetable", // Plant part used as food
    "Vegetation", // Plant life
    "Veery", // Bird
    "Velociraptor", // Dinosaur
    "Velvet worm",
    "Vendace", // Fish
    "Verdin", // Bird
    "Vervet monkey",
    "Vicuña", // Camelid
    "Vine", // Plant type
    "Vinegaroon", // Arachnid
    "Viper", // Snake
    "Viperfish",
    "Vireo", // Bird
    "Virginia opossum",
    "Viviparous", // Giving live birth
    "Vixen", // Female fox
    "Vole", // Rodent
    "Volcano rabbit",
    "Vulture",
    "Wahoo", // Fish
    "Wake-robin", // Trillium plant
    "Wallaby", // Marsupial
    "Wallaroo", // Marsupial
    "Walleye", // Fish
    "Walrus",
    "Wandering albatross",
    "Wapiti", // Elk
    "Warbler", // Bird
    "Warthog", // Pig
    "Wasp",
    "Water", // Substance
    "Water beetle",
    "Water boatman", // Insect
    "Water buffalo",
    "Water bug", // Insect
    "Water dragon", // Lizard
    "Water moccasin", // Cottonmouth snake
    "Water monitor", // Lizard
    "Water rail", // Bird
    "Water scorpion", // Insect
    "Water shrew",
    "Water snake",
    "Water spider",
    "Water strider", // Insect
    "Water vole",
    "Waterbuck", // Antelope
    "Waterfall", // Natural feature
    "Wattle", // Bird feature / Acacia tree
    "Wattlebird",
    "Waxwing", // Bird
    "Weasel",
    "Weather", // Atmospheric conditions
    "Weaver", // Bird / Spider
    "Weaver ant",
    "Web", // Spider web
    "Weevil", // Beetle
    "Well", // Water source
    "Welsh Corgi", // Dog
    "West Highland White Terrier", // Dog
    "Western diamondback rattlesnake",
    "Wetland", // Habitat
    "Whale",
    "Whale shark",
    "Wheatear", // Bird
    "Whelk", // Mollusk
    "Whiff", // Fish
    "Whimbrel", // Bird
    "Whip snake",
    "Whippet", // Dog
    "Whip-poor-will", // Bird
    "White admiral", // Butterfly
    "White bass", // Fish
    "White crappie", // Fish
    "White rhinoceros",
    "White stork",
    "White wagtail", // Bird
    "White whale", // Beluga whale
    "White-faced capuchin",
    "White-faced heron",
    "White-tailed deer",
    "Whitefish",
    "Whiting", // Fish
    "Whydah", // Bird
    "Wild boar",
    "Wildcat",
    "Wildebeest", // Gnu
    "Wilderness", // Natural environment
    "Wildlife", // Wild animals
    "Willow", // Plant
    "Willow ptarmigan",
    "Willow warbler",
    "Wind", // Air movement
    "Wing", // Appendage for flight
    "Winter", // Season
    "Wireworm", // Beetle larva
    "Wisent", // European bison
    "Wisteria", // Plant
    "Wolf",
    "Wolf eel",
    "Wolf spider",
    "Wolverine",
    "Wombat",
    "Wood", // Material from trees
    "Wood ant",
    "Wood bison",
    "Wood mouse",
    "Wood pigeon",
    "Wood stork",
    "Wood turtle",
    "Wood wasp",
    "Woodchuck", // Groundhog
    "Woodcock",
    "Woodcreeper", // Bird
    "Woodlouse", // Isopod
    "Woodpecker",
    "Woodrat",
    "Woolly bear caterpillar",
    "Woolly mammoth", // Extinct
    "Woolly monkey",
    "Woolly rhinoceros", // Extinct
    "Worm",
    "Worm snake",
    "Wrasse", // Fish
    "Wren",
    "Wryneck", // Bird
    "Xantus's hummingbird",
    "Xenarthra", // Mammal group (anteaters, sloths, armadillos)
    "Xenops", // Bird
    "Xerus", // Ground squirrel
    "X-Ray Tetra", // Fish
    "Xylem", // Plant tissue
    "Yak",
    "Yapok", // Water opossum
    "Yard", // Measurement / Area around house
    "Yarrow", // Plant
    "Yeast", // Fungus
    "Yellow baboon",
    "Yellow perch", // Fish
    "Yellow-bellied marmot",
    "Yellow-billed cuckoo",
    "Yellow-billed stork",
    "Yellowfin tuna",
    "Yellowhammer", // Bird
    "Yellowjacket", // Wasp
    "Yellowlegs", // Bird
    "Yellowthroat", // Bird
    "Yew", // Plant
    "Yike", // Sound (context?)
    "Yolk", // Egg part
    "Yorkshire Terrier", // Dog
    "Yosemite toad",
    "Yucca", // Plant
    "Zander", // Fish
    "Zebra",
    "Zebra duiker", // Antelope
    "Zebra finch", // Bird
    "Zebra mussel",
    "Zebra shark",
    "Zebu", // Cattle
    "Zenith", // Highest point (context?)
    "Zeren", // Antelope
    "Zibeline", // Sable fur color
    "Zokor", // Rodent
    "Zone", // Area
    "Zonkey", // Zebra-donkey hybrid
    "Zonure", // Lizard
    "Zooplankton", // Animal plankton
    "Zorilla", // Mammal (striped polecat)
    "Zorse", // Zebra-horse hybrid
    "Zosteropidae", // White-eye bird family

];
